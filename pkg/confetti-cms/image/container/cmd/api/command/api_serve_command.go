...
var ApiRoutes = []handler.Route{
	...
	handler.New("GET /images/", controller.Show),
	handler.New("POST /images", controller.Store).AppendMiddleware(auth.Middleware("images/store")),
	handler.New("PATCH /sources", source.Store).AppendMiddleware(auth.Middleware("source/update")),
}
