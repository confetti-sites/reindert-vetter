                        ...
                        <ul class="max-w-md space-y-2 text-gray-500 list-inside">
                            ${() => this.statuses.map(status => html`
                                <li class="flex items-center">
                                    ${this.#getIcon(status)}
                                    <div class="${status.state === this.state.loading && length === 1 ? 'animate-pulse ml-4 text-pretty' : 'ml-4 text-pretty'}">
                                        ${status.title}
                                    </div>
            ...
            #getIcon(status) {
                ...
                switch (status.state) {
                    ...
                    case this.state.success:
                        icon = `<svg class="w-8 h-8 me-2 text-emerald-500 shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                ...
                return `<div class="w-8 h-8">${icon}</div>`;
            }
